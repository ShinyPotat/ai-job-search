...
- [x] 3.1 Create `tests/helpers.test.ts` — parseCardEntry, parseSearchResults, parseJobDetail, extractJkHashes, withRetry, request-timeout (merged parsing + retry tests)
- [x] 3.2 Create `tests/search.test.ts` — runSearch E2E with mocked fetchSearchPage, filterByAge/limit
- [x] 3.3 Create `tests/detail.test.ts` — runDetail with mocked defuddleFetch
- [x] 3.4 Create `tests/cli.test.ts` — each error code via bun run, assert stderr JSON + exit 1
